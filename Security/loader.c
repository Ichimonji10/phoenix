...
/*! \file loader.c Loads an exe file and launches it.
    ...
    This program will search through a FAT file system on a disk and display
    all the exe files in a menu. If there is only one exe, it will load that
    file.  If there is more than one exe it will wait for the user to input a
    number for the file that they want. The exe file will then be loaded in to
    memory and executed.
 */
...
/*! The given string must be null terminated. The null byte is not considered
    part of the string's length.
...
/*! Fills in a boot_sector_info structure with all the information gathered
...
int get_boot_sector_info(struct boot_sector_info *bsi)
{
  ...
  else
  {
    ...
    bsi->number_of_root_sectors = bsi->number_of_root_directory_entries * bsi->bytes_per_entry / bsi->bytes_per_sector;
...
/*! Used to get information from the disk and store it in a buffer or
    somewhere in memory.
    ...
    \return Zero if unsuccessful.
 */
int read_write_sector(int segment, unsigned short offset, struct cylinder_head_sector chs, int number_of_sectors)
{
  union REGS r;
  struct SREGS s;
  int tries = 0;
...
/*! Fills in a disk_location structure with all the information needed to
    read from the disk at a specific logical sector.
 ...
 */
void get_disk_location(struct cylinder_head_sector *chs, int logical_sector, struct boot_sector_info bsi)
{
...
/*! Searches the root directory for exe files and displays them in a menu for
    the use to choose from. If there is only one exe file, it will be returned
    automatically.
...
int load_file(short starting_cluster, int file_start, struct boot_sector_info bsi)
{
  ...
  if (bsi.total_sectors_in_logical_volume / bsi.sectors_per_cluster < 4087) {
    ...
    do {
      ...
      i++;
      if ((cluster * 3) % 2 == 0) {
        cluster = ((FAT12_buffer[cluster*3/2+1]<<8)+FAT12_buffer[cluster*3/2])&0x0FFF;
      }
      else {
        cluster = (((FAT12_buffer[cluster*3/2+1]<<8)+FAT12_buffer[cluster*3/2])>>4)&0x0FFF;
      }
    } while(cluster < 0xFF8);
    //printf("%04X\n", cluster);
  } else {
...
/*! Fills in a exe_header_info structure with all the information gathered
    from the boot sector.
...
/*! The PSP might not be needed, but is partially filled just in case
...
/*! All the pointers in the file are fixed to point to
    their actual memory locations.
