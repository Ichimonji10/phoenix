...
/*! \file vga.c Provides VGA graphics support functions.
    ...
    displaying VGA graphics. The mode must be set to VGA_MODE in order for the graphics
    to be displayed.
 */
...
/*! Uses Bresenham's line-drawing algorithm, which uses no multiplication or division.
    Based on code by David Brackeen.
...
//! Fills an area
/*! Calls a recursive fill function using color of target
    pixel as the empty color.
...
//! Display an image
/*! Display a bmp image array. Use the tool bmp2hex and
    add the image to image.c.
...
/*! The color chosen as the transparent color will not be printed to
    the screen. The background will then show through.
