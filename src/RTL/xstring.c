...
/*! \file xstring.c Basic string handling functions.
    ...
    This file contains string handling facilities that reproduce the string
    functions in the C standard. Since using the standard C run time library
    inside Phoenix is not supported, these important functions must be
    duplicated here. Note that the names have been chosen to avoid conflicts
    with standard names in case both the Phoenix run time library and the C
    standard run time library ever end up coexisting in the same program.
 */
...
/*! The two strings must be null terminated. The comparison is done in a case
    sensitive manner.
    
    \param string1 The first string to compare.
    \param string2 The second string to compare.
    \return Zero if the strings are the same, a value greater than zero if the
    first string is "greater" than the second, and a value less than zero if
